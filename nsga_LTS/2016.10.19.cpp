...
const int MAXN = 2000;
int number_of_tasks;
int number_of_machines;//改机器时要改实例8,14
int m;
int n;
...
int pop, gen;
int isdep[cycle][MAXN << 2];//被i依赖的个数
int todep[cycle][MAXN << 2];//i依赖的任务数
int taskIndex[MAXN];// 记录每个机器所匹配到的位置
bool taskUsed[MAXN << 2];
set<int> doneSet;
...
double t[MAXN << 2][MAXN << 2];
struct time_table
{
    double time;
    int id;
} time_in_machine[MAXN << 2];
...
struct Individual
{
    vector<int> machine[MAXN << 2]; //save permutation of the tasks
...
vector<Individual> ElistCollection[MAXN];
