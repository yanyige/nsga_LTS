...
bool flag;
int main(){
    freopen("FFT_loop-compare-rev", "w", stdout);
    FILE *fp;
    int k = 0;
    if( (fp=fopen("FFT_loop-1-30-1000-nsga2-machine2-全部原解.txt","rt"))!=NULL) {
        while( !feof(fp)) {
            fscanf(fp,"%s",str);
            flag = true;
            if(str[0] == '['){
    ...
    int k1 = 0;
    if( (fp=fopen("FFT_loop-1-30-1000-machine2.txt","rt"))!=NULL) {
        while( !feof(fp)) {
            fscanf(fp,"%s",str);
            flag = true;
            if(str[0] == '['){
