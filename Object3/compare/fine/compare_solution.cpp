...
bool flag;
int main(){
    freopen("fine-machine4.txt", "w", stdout);
    FILE *fp;
    int k = 0;
    if( (fp=fopen("fine-grainedH264-1-30-400-MACHINE4-NEWMOHA-3object-3600s.txt","rt"))!=NULL) {
        while( !feof(fp)) {
            fscanf(fp,"%s",str);
            flag = true;
            if(str[0] == '['){
    ...
    int k1 = 0;
    if( (fp=fopen("fine-grainedH264-1-30-400-MACHINE4-NORESET-2object-3600.txt","rt"))!=NULL) {
        while( !feof(fp)) {
            fscanf(fp,"%s",str);
            flag = true;
            if(str[0] == '['){
