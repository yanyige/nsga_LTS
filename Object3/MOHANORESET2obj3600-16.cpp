...
void evaluate_objective(Individual *i)
{
    ...
    i->comtime=avg*mycycle;//时间花费的一部分
    avg /= m;
    for(int j = 0 ; j < m ; j ++)
    {
        ...
        for(int ii=0; ii<m; ii++)
        {
            ...
            if(isdepcopy[flag_end[ii]][position]==0)
            {
                ...
                {   //需要通信
                    ...
                    {   flag_com[ii]=1;
                        Eb+=c[kk][position]*Rb;
                        ltemp=taoend[flag_end[ii]][kk]+c[kk][position]+cominitcost;//需要cominitcost的初始化总线
                        if(ltemp>Lstart)
                        {
                            Lstart=ltemp;
                        }
...
int compareIndividual(Individual *a, Individual *b) {
    ...
    if(a->makespan == b->makespan && a->workload == b->workload ) return 0;//a=b
...
//int main(){
    ...
    strcat(outPutFile, "-MACHINE16-NORESET-2object-3600");
