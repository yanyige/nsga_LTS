...
bool updateElistCollection(Individual *i) {
    bool flag = false;
    bool changed = false;
    vector<Individual>::iterator iter;
...
//    }
    ...
    while(t < gen)
    {
        ...
        for(vector<Individual>::iterator iter = Front[1].begin(); iter != Front[1].end(); ++ iter) {
            ...
            if(nonUpdateTimes > resetTimes) {
                printf("updateTimes = %d\n", nonUpdateTimes);
                init();
                P_size = 0;
                now_rank = 1;
                make_new_pop(Collection, pop);
                ...
                {
                    evaluate_objective(&Collection[i]);
                    printf("makespan = %lf workload = %lf energy = %lf\n", Collection[i].makespan, Collection[i].workload, Collection[i].energy);
                }
...
//int main(){
    ...
    strcat(outPutFile, "-MACHINE4-k2-3object-");
