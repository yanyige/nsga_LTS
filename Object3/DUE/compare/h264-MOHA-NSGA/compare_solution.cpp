...
bool flag;
int main(){
    freopen("fine-machine4-rand10.txt", "w", stdout);
    FILE *fp;
    int k = 0;
    if( (fp=fopen("fine-grainedH264-10-30-400-MOHA-3object-MACHINE4-3600s-RAND10.txt","rt"))!=NULL) {
        while( !feof(fp)) {
            fscanf(fp,"%s",str);
            flag = true;
            if(str[0] == '['){
    ...
    int k1 = 0;
    if( (fp=fopen("fine-grainedH264-10-30-400-NSGA2-3object-MACHINE4-3600s-RAND10.txt","rt"))!=NULL) {
        while( !feof(fp)) {
            fscanf(fp,"%s",str);
            flag = true;
            if(str[0] == '['){
