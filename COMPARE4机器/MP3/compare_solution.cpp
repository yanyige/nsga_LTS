...
bool flag;
int main(){
    freopen("mp3_decoder-compare-rev.txt", "w", stdout);
    FILE *fp;
    int k = 0;
    if( (fp=fopen("mp3_decoder-1-30-1000-nsga2-machine4-全部原解.txt","rt"))!=NULL) {
        while( !feof(fp)) {
            fscanf(fp,"%s",str);
            flag = true;
            if(str[0] == '['){
    ...
    int k1 = 0;
    if( (fp=fopen("mp3_decoder-1-30-1000-machine4.txt","rt"))!=NULL) {
        while( !feof(fp)) {
            fscanf(fp,"%s",str);
            flag = true;
            if(str[0] == '['){
